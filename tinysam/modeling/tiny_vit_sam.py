...
def create_dct_masks_triangle(H, W, device='cuda'):
    u = torch.arange(H, device=device)
    v = torch.arange(W, device=device)
    U, V = torch.meshgrid(u, v, indexing='ij')

    threshold_small = min(H, W) // 16

    # 创建三种频率掩码，对应图片中的三种划分方式
    mask_list = nn.ParameterList([
        # 第一个矩形：左上角小三角形为1，其余为0
        ((U + V) < threshold_small).float().unsqueeze(0).unsqueeze(0),
        # 第二个矩形：左上角小三角形为0，其余为1
        ((U + V) >= threshold_small).float().unsqueeze(0).unsqueeze(0)
    ])
    return mask_list
...
class TinyViTBlock(nn.Module):
    ...
    def forward(self, x, o):
        ...
        if H == self.window_size and W == self.window_size:
            ...
            # window partition
            o = o.view(B, nH, self.window_size, nW, self.window_size, oC).transpose(2, 3).reshape(
                B * nH * nW, self.window_size * self.window_size, oC)
            x, o = self.attn(x, o)
            # window reverse
            x = x.view(B, nH, nW, self.window_size, self.window_size,
                       C).transpose(2, 3).reshape(B, pH, pW, C)
            o = o.view(B, nH, nW, self.window_size, self.window_size,
                       oC).transpose(2, 3).reshape(B, pH, pW, oC)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if downsample is not None:
            self.downsample = downsample(
                input_resolution, dim=dim, out_dim=out_dim, activation=activation)
            self.odown = nn.MaxPool2d(kernel_size=2, stride=2)
        else:
            self.downsample = None
    ...
    def forward(self, x, o):
        ...
        if self.downsample is not None:
            b, l, c = o.shape
            h = math.isqrt(l)
            o = o.reshape(b, h, h, c).permute(0, 3, 1, 2)
            #target_channels = self.downsample.out_dim
            if l == 1024:
                o = self.odown(o)
            #remainder = target_channels % c
            b, c, h, w = o.shape
            o = o.permute(0, 2, 3, 1).reshape(b, h*w, c)
            x = self.downsample(x)
        return x, o
...
class TinyViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        width = height
        x_2ud = x.reshape(batch, height, width, dim).permute(0, 3, 1, 2)
        o0 = dct.dct_2d(x_2ud, norm='ortho')
        mask_list = create_dct_masks_triangle(o0.shape[-2], o0.shape[-1])
        o0 = o0 * mask_list[1]
        o0 = dct.idct_2d(o0, norm='ortho')
        o0 = o0.permute(0, 2, 3, 1).reshape(batch, height * width, dim)'''

        start_i = 1
        #o = o0
        o = torch.zeros([x.shape[0], x.shape[1], 64], dtype=x.dtype, device=x.device)
